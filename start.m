...
idx=find(Y==1);
idx1=find(Y==-1);
plot(X(1,idx),X(2,idx),'ro',X(1,idx1),X(2,idx1),'b+',Xtest(1,:),Xtest(2,:),'gd');  %Inserted to solve the absence of gplotmatrix
...
solTsvm=tsvm(X,Y,Xtest,C,Ctest,numPlus,epsilon,type_nummer,par);
...
if length(X(:,1))==2
    ...
    %SVMplot(X,Y,X,Y,tsvmalpha,tsvmbias,40,type_nummer,par,epsilon);
    title('training set');
end
...
%Yclass=sign(fsvmclass(X,Y,type_nummer,epsilon,tsvmalpha,tsvmbias,X,par));
...
%      figure(2),subplot(1,2,2),SVMplot(Xtest,Ytest,X,Y,tsvmalpha,tsvmbias,40,type_nummer,par,epsilon),
        title('test set');    
    end 
    latent=fsvmclass(X,Y,type_nummer,epsilon,alpha1,b1,Xtest,par);
    %latent=fsvmclass(X,Y,type_nummer,epsilon,tsvmalpha,tsvmbias,Xtest,par);
