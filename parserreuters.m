...
X = zeros(9947,600);%Initialize a vector of 9947 rows times 600 Column
X=sparse(X); %Declare Sparse Xtest to improve his manage
Y =zeros(600,1);
linenumber = 0;
%while (linenumber < 11),
while (~feof(fid)), %While no end of file
    ...
    if (length(linet)>0)
        if (linet(1) ~= '#')%If is no comment
            ...
            Y(linenumber-1,:)=yy%Add to the Y vector
            xx = xe;
            ...
            X(:,linenumber-1)=xx;%if is part of the test example is stored in his matrix
            Xtest=[X xx];%otherwise is stored in the Matrix of training examples
        end
    end
end